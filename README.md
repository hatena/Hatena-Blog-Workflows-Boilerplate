...
default:
  local_root: entries
```
2. GitHub リポジトリの設定 「`Secrets and variables` > `Repository variables`」 から以下のVariableを登録する
    - Name: `BLOG_DOMAIN` 
    - Value: ブログのドメインを指定してください 例) staff.hatenablog.com
3. GitHub リポジトリの設定 「`Secrets and variables` > `Repository Secrets`」 から以下のSecretを登録する
    - Name: `OWNER_API_KEY`
